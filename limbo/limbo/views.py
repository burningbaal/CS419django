...
def goToMethod(request):
	...
	if not request.user.is_authenticated:
		return redirect(logoutLimbo)
	if not request.user.has_perm('limbo.change_version') or not request.user.has_perm('limbo.change_method'):
...
def editMethods(request):
	if not request.user.is_authenticated:
		return redirect(logoutLimbo)
	if not request.user.has_perm('limbo.change_method'):
		return redirect(indexLimbo, message='Sorry, you do not have permission to edit Methods.  \n')
	formSet = modelformset_factory(Method, exclude=('id',), extra=1)
	helper = MethodFormSetHelper()
	helper.add_input(Submit("submit", "Save"))
...
def editInstrTypes(request):
	if not request.user.is_authenticated:
		return redirect(logoutLimbo)
	if not request.user.has_perm('limbo.change_instrument'):
		...
		for vers in Instr_Version.objects.all():
			if str(vers.FK_version.id) in postedVersions:
				...
				if not Instr_Version.objects.filter(FK_instrument=assetId, FK_version=vers.FK_version).exists():
					...
					userId = curUser.id
					curProfile = UserProfile.objects.get(user=curUser) 
					validation, created = Instr_Version.objects.get_or_create(FK_instrument=asset, FK_version=vers.FK_version, timestamp=datetime.now(), validating_user=curProfile)
				else:
...
def editServer(request):
	if not request.user.is_authenticated:
		return redirect(logoutLimbo)
	if not request.user.has_perm('limbo.change_serverConfig'):
