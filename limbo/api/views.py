...
def addUsageHistory(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			newUse = form.save()
			message = HttpResponse('{"Added":["user": "' + str(newUse.FK_user.user.username) + '","version":"' + str(newUse.FK_version.version_number) + '","instrument":"' + str(newUse.FK_instrument.name) + '","time":"' + str(newUse.timestamp) + '"]}')
